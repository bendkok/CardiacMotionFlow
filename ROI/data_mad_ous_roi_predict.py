...
def data_mad_ous_roi_predict(use_info_file=True, delete=False):
    data_dir = "C:/Users/benda/Documents/Jobb_Simula/MAD_motion/MAD_OUS/MAD_OUS_sorted/" #config.acdc_data_dir
    ...
    if delete:
        ...
        if os.path.exists(os.path.join(out_dir, 'MAD_OUS_mask_original_2D')):            
            shu.rmtree(os.path.join(out_dir, 'MAD_OUS_mask_original_2D'))
    
    #we can either use a file with info on the different paitients, or we can auto-parse the directory
    if not use_info_file:        
        
        # all_subjects = ['{}'.format(str(x)) for x in range(100)]
        all_subjects = sorted(os.listdir(data_dir), key=key_sort_files)
    ...
    for s,subject in enumerate(all_subjects):
        ...
        if not use_info_file:
            ...
            # subject_dir = (data_dir+'{}').format(subject) #change
            subject_dir = data_dir+subject #change
            # print(subject_dir)
            # exit
        ...
        subject_predict_dir = os.path.join(out_dir, 'MAD_OUS_mask_original_2D', subject)
        ...
        #Here they tried to get only the data for ED. MAD OUS dosen't record when that is, so I've just used 0 instead
...
if __name__ == '__main__':
    # out = data_mad_ous_roi_predict(False)
    # print(out)
    # for o in out[0]:
    #     print(o)
