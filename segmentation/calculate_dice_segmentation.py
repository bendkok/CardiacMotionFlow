...
def calculate_dice_segmentation(dataset = 'acdc'):
    ...
    if dataset == 'acdc':
        ...
        for f in range(1,6):
            ...
            seq_segs_pre += [seq_segs[se] + seq_segs[se+1] for se in range(len(seq_segs))[::2]]
            # seq_segs_pre += seq_segs
        # seq_context_imgs, seq_context_segs, seq_imgs, seq_segs_pre = data_lvrv_segmentation_propagation_acdc(mode = 'predict', fold = fold)
        # seq_segs_pre = seq_segs_pre[::2]
        seq_segs_pre = np.sort(seq_segs_pre)
        seq_segs_gt = [[a.replace('predict_lvrv2_', 'crop_2D_gt_', 1) for a in b ] for b in seq_segs_pre]
        seq_segs_gt = [[a.replace('predict_2D', 'crop_2D', 1) for a in b ] for b in seq_segs_gt]
    ...
    predict_sequence = len(seq_segs_pre)
    ...
    for i in range(predict_sequence):
        ...
        segs_pre = seq_segs_pre[i]
        #the segs_gt is not grouped 
        segs_gt = seq_segs_gt[i]
        # num_processed += len(segs_pre)
        ...
        # segs_pre_im = iio.imread(segs_pre)
        # segs_gt_im = iio.imread(segs_gt)
        
        curr_dice2 = dice_coef2(segs_gt_im, segs_pre_im).numpy()
    
        segs_pre_im = tf.where(segs_pre_im > 0.5, K.ones_like(segs_pre_im), K.zeros_like(segs_pre_im))
        segs_gt_im = tf.where(segs_gt_im > 0.5, K.ones_like(segs_gt_im), K.zeros_like(segs_gt_im))
        curr_dice = dice_coef(segs_gt_im, segs_pre_im).numpy()
        
        dice_scores.append(curr_dice)
        dice_scores2.append(curr_dice2)
        
    return dice_scores,dice_scores2
